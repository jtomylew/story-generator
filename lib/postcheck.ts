...
export function postCheck(res: GenerateResType, level: string): void {
  ...
  if (!res.questions || res.questions.length !== 2) {
    throw new Error(`Expected exactly 2 questions, got ${res.questions?.length || 0}`);
  }
  ...
  for (let i = 0; i < res.questions.length; i++) {
    ...
    if (question.length < 10) {
      throw new Error(`Question ${i + 1} is too short (${question.length} chars). Questions should be at least 10 characters long.`);
    }
